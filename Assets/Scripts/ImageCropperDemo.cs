...
namespace ImageCropperNamespace
{
	public class ImageCropperDemo : MonoBehaviour
	{
		...
		private IEnumerator TakeScreenshotAndCrop()
		{
			...
			Texture2D sc = new Texture2D(2, 2);
			sc.LoadImage(ImageAsset.bytes);

			//Texture2D screenshot = new Texture2D( Screen.width, Screen.height, TextureFormat.RGB24, false );
			//screenshot.ReadPixels( new Rect( 0, 0, Screen.width, Screen.height ), 0, 0 );
			//screenshot.Apply();

			ImageCropper.Instance.Show( sc, ( bool result, Texture originalImage, Texture2D croppedImage ) =>
			{
				...
				if( result )
				{
					...
					if( croppedImage.height <= croppedImage.width )
						size = new Vector2( 350f, 350f * ( croppedImage.height / (float) croppedImage.width ) );
					else
						size = new Vector2( 350f * ( croppedImage.width / (float) croppedImage.height ), 350f );
					croppedImageHolder.rectTransform.sizeDelta = size;

					gameManager.ReceiveCroppedImage(croppedImage);
